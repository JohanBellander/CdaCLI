...
export function buildOnboardingGuide() {
    ...
    lines.push("");
    lines.push("Do not continue until each command succeeds in this exact order. After every command, paste the command plus its outcome into your transcript before moving on.");
    lines.push("");
    let stepNumber = 1;
    ...
    lines.push("**Reminder**: Each `--phase` run revalidates every prior phase cumulatively. Do not skip ahead until the current phase passes `cda run --phase <name> --exec`.");
    lines.push("");
    lines.push("## Evidence Checklist");
    lines.push("");
    lines.push("- After each `cda agent --phase <name> --dry-run`: record the `run_id`, link to the archived prompt, and capture the phase objective summary in your transcript.");
    lines.push("- After each `cda run --phase <name> --exec`: capture the process exit status, the emitted `run_id`, and at least the opening lines of the agent's report.");
    ...
    lines.push("- Re-run `cda run --phase <current-phase> --exec` and fix the regression immediately. Phase prompts are cumulative checkpoints, not isolated sandboxes.");
    lines.push("");
    lines.push("**Can I skip phases?**");
    lines.push("- No. Each phase assumes the earlier layers are complete and validated. Skipping phases results in incoherent architecture and failing prompts.");
    lines.push("");
    lines.push("**When is the legacy all-at-once workflow appropriate?**");
    lines.push("- Maintenance or refactor sessions on mature codebases, single-constraint investigations, or auditor reviews. New codebases must follow the phased path.");
    lines.push("");
    ...
    lines.push("");
    lines.push("- You MUST NOT declare success until `cda run --phase <name> --exec` completes successfully for the current phase and the evidence is recorded in your transcript.");
    ...
    lines.push("");
    lines.push("Do NOT wait for the user to prompt validation. Treat `cda run --phase <name> --exec` as your automated test suite—run it proactively to catch architectural drift early.");
    lines.push("");
