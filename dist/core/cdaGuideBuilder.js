...
function addConstraintSummaryTable(lines, ordered) {
    ...
    for (const constraint of ordered) {
        lines.push(`| ${constraint.meta.enforcementOrder} | \`${constraint.meta.id}\` | ${constraint.meta.name} | ${sanitize(constraint.sections.PURPOSE)} |`);
    }
    lines.push("");
}
function addPerConstraintGuidance(lines, ordered) {
    ...
    lines.push("");
    for (const constraint of ordered) {
        lines.push(`### ${constraint.meta.name} (\`${constraint.meta.id}\`, order ${constraint.meta.enforcementOrder})`);
        lines.push("");
