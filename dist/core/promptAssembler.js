...
export function assemblePrompt(options) {
    const { runId, instructionText, agentName, promptPreamble, postscript, legacyFormat = false, disabledConstraints = [], phaseContext, } = options;
    ...
    if (!legacyFormat) {
        ...
        if (phaseContext) {
            lines.push("========================================");
            lines.push(`CDA AGENT PROMPT - ${phaseContext.metadata.phase_label.toUpperCase()}`);
            lines.push("========================================");
            lines.push("");
            lines.push(`Phase Mode: ${phaseContext.metadata.phase_mode} (Phase ${phaseContext.metadata.phase_number} of ${PHASES.length})`);
            lines.push(`Included Phases: ${phaseContext.metadata.included_phases.join(", ")}`);
            lines.push(formatConstraintLine("Constraints in This Phase", phaseContext.metadata.constraints_in_phase, phaseContext.phaseActiveCount));
            lines.push(formatConstraintLine("Cumulative Constraints", phaseContext.metadata.cumulative_constraints, phaseContext.cumulativeActiveCount, formatPhaseRange(phaseContext.metadata.included_phases)));
            lines.push(phaseContext.metadata.next_phase
    ...
    if (phaseContext) {
        ...
        lines.push("");
        lines.push(`Key principles for ${phaseContext.metadata.phase_label}:`);
...
function formatConstraintLine(label, definedCount, activeCount, suffix) {
    const base = `${label}: ${definedCount}`;
    const range = suffix ? ` (${suffix})` : "";
    const activeNote = activeCount === definedCount
        ? ""
        : ` (active: ${activeCount})`;
    return `${base}${range}${activeNote}`;
}
function formatPhaseRange(phases) {
    if (phases.length === 0) {
        return undefined;
    }
    if (phases.length === 1) {
