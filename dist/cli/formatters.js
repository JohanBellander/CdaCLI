...
export function formatBatchInstructionPackage(pkg) {
    const lines = [];
    ...
    lines.push("INSTRUCTION PACKAGE ONLY - NO SOURCE ANALYSIS PERFORMED - AGENT MUST EXECUTE DETECTION STEPS");
...
export function formatSingleInstructionPackage(pkg) {
    const lines = [];
...
export function formatLegacyBatchInstructionPackage(pkg) {
    const lines = [];
...
export function formatLegacySingleInstructionPackage(pkg) {
    const lines = [];
...
function formatConstraintBlock(block, options = {}) {
    const lines = [];
    if (options.legacy) {
...
function formatLegacyBatchReportTemplate(pkg) {
...
function formatLegacySequentialReportTemplate(pkg) {
...
function formatBatchReportTemplate(pkg) {
    const { reportTemplate } = pkg;
    const lines = [];
    ...
    lines.push(`  all_constraints_evaluated: ${reportTemplate.successConditions.allConstraintsEvaluated}`);
    lines.push(`  no_remaining_violations: ${reportTemplate.successConditions.noRemainingViolations}`);
    lines.push("self_audit:");
    lines.push(`  all_constraints_present: ${reportTemplate.selfAudit.allConstraintsPresent}`);
    lines.push(`  all_required_fields_populated: ${reportTemplate.selfAudit.allRequiredFieldsPopulated}`);
    lines.push(`  revalidation_attempts_documented: ${reportTemplate.selfAudit.revalidationAttemptsDocumented}`);
    lines.push(`  schema_conformance: ${reportTemplate.selfAudit.schemaConformance}`);
    lines.push(`agent_execution_signature: ${formatNullable(reportTemplate.agentExecutionSignature)}`);
...
function formatSequentialReportTemplate(pkg) {
    const template = pkg.reportTemplate;
    const lines = [];
    ...
    lines.push(`  revalidation_attempts_documented: ${template.selfAudit.revalidationAttemptsDocumented}`);
