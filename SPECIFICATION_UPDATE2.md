...
- `mode`: one of `stdin` (MVP only), future: `arg`, `file`.
...
## 9. Execution Flow (MVP - stdin mode only)
...
5. Spawn child process: `command` + `args`.
6. Pipe prompt to stdin of child.
...
| External command not found | Error | `FATAL: Unable to spawn 'gh'. Is GitHub CLI installed?` |
| Unsupported mode value | Error | `CONFIG_ERROR: Unsupported agent mode 'file' (MVP provides 'stdin' only).` |
...
1. Additional modes: `arg`, `file` with placeholders.
...
5. Implement execution path (stdin only) & flags.
...
- Copilot CLI may change flags; we rely on current stable `gh copilot chat` interface.
...
- `Agent Execution Mode`: The invocation strategy (stdin for MVP; future arg/file).
