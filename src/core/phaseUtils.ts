...
const PHASE_KEY_PRINCIPLES: Record<Phase, string[]> = {
  foundation: [
...
export function assertValidPhase(phaseName: string): Phase {
  const normalized = phaseName.trim().toLowerCase() as Phase;
  if ((PHASES as readonly string[]).includes(normalized)) {
    return normalized;
  }
  throw createError(
    "CONFIG_ERROR",
    `Unknown phase '${phaseName}'. Valid options: ${PHASES.join(", ")}.`,
  );
}
...
export function getPhaseIndex(phase: Phase): number {
  ...
  if (index === -1) {
    throw createError(
      "CONFIG_ERROR",
      `Unknown phase '${phase}'. Valid options: ${PHASES.join(", ")}.`,
    );
  }
  return index;
}

export function getPhaseConstraintIds(phase: Phase): string[] {
  const index = getPhaseIndex(phase);
  const includedPhases = PHASES.slice(0, index + 1);
  const ids = includedPhases.flatMap(
    (phaseName) => PHASE_CONSTRAINTS[phaseName] ?? [],
  );
  return Array.from(new Set(ids));
}

export function getConstraintsForPhase(
  phase: Phase,
  allConstraints: ConstraintMeta[],
): ConstraintMeta[] {
  const ids = new Set(getPhaseConstraintIds(phase));
  return allConstraints.filter(
    (constraint) => constraint.isActive && ids.has(constraint.id),
  );
}

export function filterConstraintsByPhase(
  ...
  const cumulativeIds = new Set(
    includedPhases.flatMap((phaseName) => PHASE_CONSTRAINTS[phaseName] ?? []),
  );
  const phaseOnlyIds = new Set(PHASE_CONSTRAINTS[input.phase] ?? []);

  const filteredActive = input.activeConstraints.filter((doc) =>
    cumulativeIds.has(doc.meta.id),
  );
  if (filteredActive.length === 0) {
    throw createError(
      "CONFIG_ERROR",
      `Phase '${input.phase}' currently has no active constraints. Disable overrides or pick another phase.`,
    );
  }
  ...
  const allConstraintIds = new Set(
    input.allConstraints.map((doc) => doc.meta.id),
  );
  const missingConstraintIds = Array.from(cumulativeIds).filter(
    (id) => !allConstraintIds.has(id),
  );
  ...
  const info: PhaseInfo = {
    ...
    phaseActiveCount: filteredActive.filter((doc) =>
      phaseOnlyIds.has(doc.meta.id),
    ).length,
...
export function buildPhasePromptContext(info: PhaseInfo): PhasePromptContext {
  const phaseLabel = buildPhaseLabel(info.phase, info.phaseNumber);
  const objectiveLines = PHASE_OBJECTIVES[info.phase];

  const includedLabels = info.includedPhases.map((phaseName) =>
    buildPhaseLabel(phaseName, PHASES.indexOf(phaseName) + 1),
  );
...
export function formatPhaseWarnings(result: PhaseFilterResult): string[] {
  const label = buildPhaseLabel(result.info.phase, result.info.phaseNumber);
  const warnings: string[] = [];
  if (result.disabledConstraintIds.length > 0) {
    warnings.push(
      `${label} has ${result.disabledConstraintIds.length} disabled constraint(s): ${result.disabledConstraintIds.join(
        ", ",
      )}. They will be skipped until re-enabled.`,
    );
  }
  if (result.missingConstraintIds.length > 0) {
    warnings.push(
      `${label} references missing constraint ids: ${result.missingConstraintIds.join(
        ", ",
      )}. Update PHASE_CONSTRAINTS or ensure the markdown files exist.`,
    );
  }
  return warnings;
}
...
function buildValidationLines(
  ...
  if (phaseNumber === 1) {
    ...
    lines.push(
      `Previous phase (${previousLabel}) should already be validated and passing.`,
    );
  }
  return lines;
}

function formatPhaseList(labels: string[]): string {
  if (labels.length === 1) {
    return labels[0];
  }
...
function buildPhaseLabel(phase: Phase, phaseNumber: number): string {
  return `Phase ${phaseNumber}: ${PHASE_TITLES[phase]}`;
}

function buildNextSteps(info: PhaseInfo): string[] {
  ...
  if (info.nextPhase) {
    const nextLabel = buildPhaseLabel(
      info.nextPhase,
      PHASES.indexOf(info.nextPhase) + 1,
    );
    ...
    steps.push(
      `   - Focus on ${PHASE_SUMMARIES[info.nextPhase].toLowerCase()}`,
    );
    steps.push(
      `Do NOT start ${nextLabel} work while executing ${label}. Stay focused on ${PHASE_SUMMARIES[
        info.phase
      ].toLowerCase()}.`,
    );
  } else {
    steps.push(
      "4. After presentation validation passes, begin release prep and regression hardening.",
    );
  }
